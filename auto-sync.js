...
const VIRTUAL_BOARD_EXPECTED_MATCH_COUNT = 10;
  ...
  return {
    ...
    boards: boardPayloads.map((boardPayload, index) => {
      ...
      return {
        ...
        expectedMatchCount: boardPayload.expectedMatchCount ?? (boardPayload.events ?? []).length,
...
async function postFeedEventsBoard(cycle, boardPayload, lastPostedState, meta = {}) {
  ...
  const canonicalValidation = validateCanonicalBoard({
    providerEventId: boardPayload.providerEventId,
    expectedMatchCount: boardPayload.expectedMatchCount,
...
async function postCanonicalEventsForSource(cycle, source, canonicalEvents, lastPostedHashes, extra = {}) {
  ...
  const canonicalValidation = validateCanonicalBoard({
    ...
    expectedMatchCount: extra.expectedMatchCount ?? VIRTUAL_BOARD_EXPECTED_MATCH_COUNT,
    events: eventsToPost,
  });
  if (!canonicalValidation.valid) {
    logBlockedCanonicalBoard(canonicalValidation);
